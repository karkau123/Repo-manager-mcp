...
class GitHubAPIService {
   ...
   * @param {string} name - Repository name (use hyphens; no spaces)
   * @param {object} options - Optional: description, private, auto_init
  ...
  async createRepository(name, options = {}) {
    const payload = {
      ...
      ...(options.auto_init != null && { auto_init: !!options.auto_init }),
    };
    return this.makeGitHubRequest("/user/repos", {
      method: "POST",
      body: JSON.stringify(payload),
